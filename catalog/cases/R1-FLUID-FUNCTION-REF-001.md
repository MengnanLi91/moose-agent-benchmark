...
case_id: R1-FLUID-FUNCTION-REF-001
title: "Repair Broken Inlet-Function Reference"
case_kind: atomic
track: REPAIR
subcategory: R1
physics_domains:
...
# Repair Broken Inlet-Function Reference
...
This planned atomic case, **Repair Broken Inlet-Function Reference**, exercises the R1 capability.
...
- execution succeeds;
- inlet profile remains unchanged;
- pressure-drop and flow-rate regression quantities stay within tolerance.

Capabilities outside R1 are intentionally excluded from the score.

## Starting information

A velocity boundary condition references:

```text
inlet_vel
```

but the required existing Function object is:
...
- creating a new inlet profile;
- altering boundary conditions;
- changing solver settings.

## Isolation rationale

Upstream truth is supplied and downstream work is excluded so the case scores only R1.
...
- Planned public case (not created): `benchmark/public/cases/R1-FLUID-FUNCTION-REF-001/`
- Planned private evaluator (not created): `benchmark/private/cases/R1-FLUID-FUNCTION-REF-001/`
- Planned development fixtures (not created): `benchmark/example-submissions/R1-FLUID-FUNCTION-REF-001*.json`
...
The proposal used `L1-L2`; this record selects `L2` because the catalog schema requires one difficulty.
